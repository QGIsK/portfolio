<template>
    <div id="contact" class="bg-dark">
        <v-container>
            <v-row>
                <v-col
                    :cols="$vuetify.breakpoint.mdAndDown ? 12 : 8"
                    class="mx-auto mt-10 mb-15"
                    :class="$vuetify.breakpoint.mdAndDown ? ' text-center' : ''"
                >
                    <h1 class="display-1 mb-3">Contact</h1>
                    <h3 class="mb-5">
                        Looking for a web developer for your next project, or a recruiter, well trying to recruit?
                    </h3>

                    <v-row>
                        <v-col
                            :cols="$vuetify.breakpoint.mdAndDown ? 8 : 6"
                            :class="$vuetify.breakpoint.mdAndDown ? 'mx-auto text-center' : ''"
                        >
                            <h4 class="mb-3">Send me an email</h4>
                            <a href="mailto:hello@demiann.dev" style="text-decoration: none">
                                <v-card elevation="0">
                                    <v-card-text>
                                        <i class="fa fa-envelope fa-lg mx-3"></i>

                                        <span class="underline">hello@demiann.dev</span>
                                    </v-card-text>
                                </v-card>
                            </a>
                        </v-col>
                    </v-row>

                    <h4 class="my-5">Prefer something a little less formal?</h4>
                    <v-row>
                        <v-col :cols="$vuetify.breakpoint.mdAndDown ? 8 : 6" class="mx-auto">
                            <v-card elevation="0">
                                <v-card-text>
                                    <i class="fab fa-telegram fa-lg mx-3"></i>

                                    @Demiann
                                </v-card-text>
                            </v-card>
                        </v-col>
                        <v-col :cols="$vuetify.breakpoint.mdAndDown ? 8 : 6" class="mx-auto">
                            <v-card elevation="0">
                                <v-card-text>
                                    <i class="fab fa-discord fa-lg mx-3"></i>
                                    Demian#6666
                                </v-card-text>
                            </v-card>
                        </v-col>
                    </v-row>
                </v-col>
            </v-row>
        </v-container>
    </div>
</template>

<script>
export default {
    name: 'Contact',
};
</script>

<style lang="scss" scoped>
span.underline {
    text-decoration: underline;
}

.fa-discord {
    color: #7289da;
}

.fa-telegram {
    color: #0088cc;
}
</style>
